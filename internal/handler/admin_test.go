...
func TestSettingsTemplateContainsRuntimeStatusPanel(t *testing.T) {
	for _, want := range []string{"{{template \"runtimeStatusCSS\"}}", "{{template \"runtimeStatusPanel\" .}}"} {
		if !strings.Contains(templates.SettingsAdminHTML, want) {
			t.Fatalf("expected settings template contains %q", want)
		}
	}
}
