...
func InitEmbedder() {
	logx.With("component", "embedder").Info("loading BGE embedder")
	modelExists := false
	...
	if !modelExists {
		...
		logx.With("component", "embedder", "error", embedderInitError.Error()).Warn("local model files unavailable, fallback to basic rules")
		embedderModelName = "基础规则 (本地模型不可用)"
		return
	}
	globalEmbedder, embedderInitError = embedder.NewBgeOnnxEmbedder()
	if embedderInitError != nil {
		logx.With("component", "embedder", "error", embedderInitError.Error()).Error("BGE embedder initialization failed")
		embedderModelName = "基础规则 (模型加载失败)"
	} else {
		logx.With("component", "embedder").Info("BGE embedder initialized")
	}
}
...
func scan(store *storage.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 原始名称：文件夹上传时取顶层目录名，单文件取文件名
		originalName := buildUploadedOriginalName(files)
		...
		if globalEmbedder != nil && embedderInitError == nil {
			...
			if err != nil {
				logx.FromContext(r.Context()).With("component", "scan", "error", err.Error()).Warn("failed to load rules config, fallback to default embedded rules")
			...
			if user != nil {
				...
				if userLLM != nil && userLLM.Enabled {
					...
					if !ok {
						clientErr := fmt.Errorf("LLM provider unavailable")
						logx.FromContext(r.Context()).With("component", "scan", "provider", userLLM.Provider, "username", sess.Username, "error", clientErr.Error()).Warn("LLM provider unavailable")
					} else {
						client, clientErr := llm.NewClient(llm.ProviderConfig{Provider: provider.ID, Name: provider.Name, Protocol: provider.Protocol, BaseURL: provider.BaseURL, Model: provider.Model, APIKey: provider.APIKey})
						if clientErr == nil {
							llmClient = client
							logx.FromContext(r.Context()).With("component", "scan", "provider", provider.ID, "username", sess.Username).Info("LLM provider enabled for user")
						}
					}
				}
			}
			...
			evalResult, err = eval.Evaluate(context.Background(), skill)
			if err == nil {
...
func runPlugins(scanPath string) []plugins.Finding {
	...
	runtimeCfg, err := plugins.LoadRuntimeConfig(config.PluginsConfigPath())
	if err != nil {
		logx.With("component", "plugins", "path", config.PluginsConfigPath(), "error", err.Error()).Warn("failed to load runtime plugin config, fallback to env filters")
	}
	enabled, disabled := plugins.ResolvePluginFilters(config.EnabledPlugins(), config.DisabledPlugins(), runtimeCfg)
	return plugins.ExecuteAll(ctx, scanPath, registry.BuildWithFilter(enabled, disabled))
}
...
// getDefaultConfig 提供一个规则集风险等级兜底配置，当配置文件加载失败时使用。
