...
func renderHTMLEvidenceList(title string, items []string, empty string) string {
	...
	codeEvidence, contextEvidence, declarationEvidence, otherEvidence := splitRenderedEvidenceGroups(cleaned)
	var b strings.Builder
	b.WriteString("<div><strong>" + html.EscapeString(title) + ":</strong><div class=\"stack\">")
	renderEvidenceGroup(&b, "代码证据", codeEvidence)
	renderEvidenceGroup(&b, "上下文证据", contextEvidence)
	renderEvidenceGroup(&b, "声明证据", declarationEvidence)
	renderEvidenceGroup(&b, "补充说明", otherEvidence)
	b.WriteString("</div></div>")
	return b.String()
}

func splitRenderedEvidenceGroups(items []string) ([]string, []string, []string, []string) {
	codeEvidence := make([]string, 0, len(items))
	contextEvidence := make([]string, 0, len(items))
	declarationEvidence := make([]string, 0, len(items))
	otherEvidence := make([]string, 0, len(items))
	for _, item := range items {
		item = strings.TrimSpace(item)
		if item == "" || strings.HasPrefix(item, "链路:") || isWeakPlaceholderEvidence(item) {
			continue
		}
		switch classifyRenderedEvidence(item) {
		case "code":
			codeEvidence = append(codeEvidence, item)
		case "context":
			contextEvidence = append(contextEvidence, item)
		case "declaration":
			declarationEvidence = append(declarationEvidence, item)
		default:
			otherEvidence = append(otherEvidence, item)
		}
	}
	codeEvidence = filterRedundantLocationOnlyEvidence(codeEvidence)
	contextEvidence = filterRedundantLocationOnlyEvidence(contextEvidence)
	declarationEvidence = filterRedundantLocationOnlyEvidence(declarationEvidence)
	return codeEvidence, contextEvidence, declarationEvidence, otherEvidence
}

func filterRedundantLocationOnlyEvidence(items []string) []string {
	if len(items) < 2 {
		return items
	}
	richByPath := map[string]struct{}{}
	for _, item := range items {
		path, rich := evidencePathAndRichness(item)
		if path == "" || !rich {
			continue
		}
		richByPath[path] = struct{}{}
	}
	if len(richByPath) == 0 {
		return items
	}
...
func evidencePathAndRichness(item string) (string, bool) {
	trimmed := strings.TrimSpace(item)
	if trimmed == "" {
		return "", false
	}
	if strings.HasPrefix(trimmed, "位置:") {
		path := strings.TrimSpace(strings.TrimPrefix(trimmed, "位置:"))
	...
	if strings.HasPrefix(trimmed, "证据引用:") {
		body := strings.TrimSpace(strings.TrimPrefix(trimmed, "证据引用:"))
		if idx := strings.Index(body, ":"); idx > 0 {
			return strings.TrimSpace(body[:idx]), true
		}
		return body, false
	}
	if path, _, ok := tryParseInlineLocator(trimmed); ok {
		return path, true
	}
	return "", true
}

func renderEvidenceGroup(b *strings.Builder, title string, items []string) {
	if len(items) == 0 {
		return
	}
	b.WriteString("<div><strong>" + html.EscapeString(title) + ":</strong><div class=\"stack\">")
...
func classifyRenderedEvidence(item string) string {
	text := strings.ToLower(strings.TrimSpace(item))
	if text == "" {
		return "other"
	}
	if containsAny(text, []string{"skill.md", "readme", "deployment.md", "manifest", "声明语义", "技能声明"}) {
		return "declaration"
	}
	if containsAny(text, []string{"位置:", "证据引用:"}) && !looksLikeCode(text) {
		return "context"
	}
	if looksLikeCode(text) || looksLikeSourceLocator(text) {
		return "code"
	}
	return "other"
}

func isWeakPlaceholderEvidence(item string) bool {
	text := strings.ToLower(strings.TrimSpace(item))
	if text == "" {
		return true
	}
	return strings.Contains(text, "entire file (empty)") || strings.HasSuffix(text, "__init__.py") || strings.HasSuffix(text, "__init__.py (empty)")
}

func renderHTMLCodeEvidence(item string) string {
	label, body := splitCodeEvidenceLabelAndBody(item)
	if strings.TrimSpace(body) == "" || strings.EqualFold(strings.TrimSpace(body), strings.TrimSpace(label)) {
		body = normalizeEvidenceBody(item)
	}
