...
func TestAdmissionServiceRejectsIncompleteDependencies(t *testing.T) {
	svc := NewAdmissionService(nil, nil, nil, nil, nil, nil)
	if _, err := svc.CreateSkillFromReport(CreateSkillFromReportInput{ReportID: "report-001"}); err == nil || err.Error() != "admission service reports lookup is nil" {
		t.Fatalf("expected reports lookup validation error, got %v", err)
	}
	if _, err := svc.GetSkillDetail("skill-001"); err == nil || err.Error() != "admission service reports lookup is nil" {
		t.Fatalf("expected get skill detail validation error, got %v", err)
	}
	if _, err := svc.ListSkills("", 10); err == nil || err.Error() != "admission service reports lookup is nil" {
		t.Fatalf("expected list skills validation error, got %v", err)
	}
}

func TestAdmissionServiceCreateSkillFromReportLoadsReviewSignalsFromReportsDir(t *testing.T) {
	dataDir := t.TempDir()
	reportsDir := filepath.Join(dataDir, "reports")
	if err := os.MkdirAll(reportsDir, 0755); err != nil {
		t.Fatalf("mkdir reports dir: %v", err)
	...
	report := &models.Report{
		ID:       "report-relative-json",
		FileName: "fixture-skill.zip",
		FilePath: "fixture-skill_20260514_010203.docx",
		JSONPath: "fixture-skill_20260514_010203.json",
	}
	...
	out, err := svc.CreateSkillFromReport(CreateSkillFromReportInput{
		ReportID:        report.ID,
		DisplayName:     "Relative JSON Skill",
		AdmissionStatus: admissionmodel.AdmissionStatusApproved,
		ReviewDecision:  admissionmodel.ReviewDecisionPass,
		ReviewSummary:   "允许准入",
		Operator:        "admin",
	...
	if !out.Profile.NetworkAccess || !out.Profile.CommandExec || !out.Profile.SensitiveDataAccess {
		t.Fatalf("expected profile populated from reports dir json, got %+v", out.Profile)
	}
...
func TestProfileBuilderPrefersTopLevelReviewMetadata(t *testing.T) {
	reportsDir := t.TempDir()
	report := &models.Report{ID: "report-top-level", FileName: "fixture-skill.zip", FilePath: filepath.Join(reportsDir, "fixture-skill.zip"), JSONPath: "report-top-level.json"}
	topLevelTrace := &review.ReviewTrace{
		Total:     1,
		Completed: 1,
	...
	topLevelStats := []review.ReviewAgentExecutionStats{{
	...
	legacyTrace := &review.ReviewTrace{
		Total:               1,
		Completed:           1,
		CurrentFindingTitle: "旧轨迹",
	}
	legacyStats := []review.ReviewAgentExecutionStats{{
	...
	result := review.Result{
		...
		ReviewTrace:      legacyTrace,
		ReviewAgentStats: legacyStats,
	}
	data, err := json.Marshal(struct {
		Result           review.Result                      `json:"result"`
		...
		Result:           result,
		ReviewTrace:      topLevelTrace,
		ReviewAgentStats: topLevelStats,
	})
	if err != nil {
		t.Fatalf("marshal report fixture: %v", err)
	}
	...
	loaded := NewProfileBuilder().loadReviewResult(ProfileBuildInput{Report: report})
	if loaded == nil {
		t.Fatal("expected review result loaded")
	}
	if loaded.ReviewTrace == nil || len(loaded.ReviewTrace.Entries) != 1 || loaded.ReviewTrace.Entries[0].FindingTitle != "顶层轨迹" {
		t.Fatalf("expected top-level review trace preferred, got %+v", loaded.ReviewTrace)
	}
	if len(loaded.ReviewAgentStats) != 1 || loaded.ReviewAgentStats[0].Reviewer != "llm-vuln-reviewer" {
		t.Fatalf("expected top-level review stats preferred, got %+v", loaded.ReviewAgentStats)
	}
}

func TestProfileBuilderFallsBackToNestedReviewMetadata(t *testing.T) {
	reportsDir := t.TempDir()
	report := &models.Report{ID: "report-nested", FileName: "fixture-skill.zip", FilePath: filepath.Join(reportsDir, "fixture-skill.zip"), JSONPath: "report-nested.json"}
	nestedTrace := &review.ReviewTrace{
		Total:               2,
		Completed:           1,
		CurrentFindingTitle: "旧结构轨迹",
	}
	nestedStats := []review.ReviewAgentExecutionStats{{
	...
	result := review.Result{
		...
		ReviewTrace:      nestedTrace,
		ReviewAgentStats: nestedStats,
	}
	data, err := json.Marshal(struct {
		Result review.Result `json:"result"`
	}{Result: result})
	...
	if loaded.ReviewTrace == nil || loaded.ReviewTrace.CurrentFindingTitle != "旧结构轨迹" {
		t.Fatalf("expected nested review trace fallback, got %+v", loaded.ReviewTrace)
	}
	if len(loaded.ReviewAgentStats) != 1 || loaded.ReviewAgentStats[0].Reviewer != "nested-reviewer" {
		t.Fatalf("expected nested review stats fallback, got %+v", loaded.ReviewAgentStats)
	}
}
