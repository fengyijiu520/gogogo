...
type CapabilityProfile struct {
	...
	CapabilityLevels    map[string]float64 `json:"capability_levels,omitempty"`
	CapabilityScopes    map[string][]string `json:"capability_scopes,omitempty"`
...
func (p *CapabilityProfile) Normalize() {
	...
	if p.CapabilityLevels == nil {
		p.CapabilityLevels = map[string]float64{}
	}
	if p.CapabilityScopes == nil {
		p.CapabilityScopes = map[string][]string{}
	}
	for key, scopes := range p.CapabilityScopes {
		p.CapabilityScopes[key] = normalizeStringSlice(scopes)
	}
...
func (p *CapabilityProfile) HasCapabilityScope(capability, scope string) bool {
	if p == nil {
		return false
	}
	...
	for _, item := range p.CapabilityScopes[capability] {
