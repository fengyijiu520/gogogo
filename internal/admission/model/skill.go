...
type AdmissionSkill struct {
	...
	LastFixAt            int64           `json:"last_fix_at,omitempty"`
...
func (s *AdmissionSkill) Normalize() {
	...
	s.WorkflowStage = strings.TrimSpace(s.WorkflowStage)
	...
	if s.WorkflowStage == "" {
		if s.VerificationRequired {
			s.WorkflowStage = "pending_verification"
		} else {
			s.WorkflowStage = "verified"
		}
	}
}
