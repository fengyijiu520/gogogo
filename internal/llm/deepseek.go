...
func (c *deepseekClient) AnalyzeObfuscatedContent(ctx context.Context, name, content string) (*ObfuscationAnalysisResult, error) {
...
请避免把普通编码、配置序列化、测试数据、文档示例直接判定为恶意。`
	...
	reqBody := deepseekChatRequest{Model: "deepseek-chat", Messages: []deepseekChatMessage{{Role: "system", Content: systemPrompt}, {Role: "user", Content: userPrompt}}, Temperature: 0.1}
	jsonData, err := json.Marshal(reqBody)
	if err != nil {
		return nil, fmt.Errorf("序列化请求失败: %w", err)
	}
