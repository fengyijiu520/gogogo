...
func New(store *storage.Store) http.Handler {
	ti.SetPolicyBlacklistProvider(store)
	...
	activeUser := handler.RequireActiveUser(store)
	mux.HandleFunc("/dashboard", activeUser(handler.RequireAuth(handler.Dashboard(store))))
	...
	mux.HandleFunc("/runtime-help", handler.RequirePermission(store, models.PermPersonalCenter)(handler.RequireAuth(handler.RuntimeHelp(store))))
	...
	mux.HandleFunc("/admin/policy-blacklist", handler.RequirePermission(store, models.PermUserManagement)(handler.RequireAuth(handler.PolicyBlacklist(store))))
	...
	mux.HandleFunc("/settings", handler.RequirePermission(store, models.PermPersonalCenter)(handler.RequireAuth(handler.SettingsPage(store))))
	...
	mux.HandleFunc("/api/runtime/status", handler.RequireAPIPermission(store, models.PermPersonalCenter)(handler.RequireAuth(handler.RuntimeStatusAPI(store))))
	// 新增：用户 LLM 配置 API
	...
	mux.HandleFunc("/api/admin/llm/providers", handler.RequireAPIPermission(store, models.PermUserManagement)(handler.RequireAuth(handler.LLMProvidersHandler(store))))
	...
	mux.HandleFunc("/api/rules/validate", handler.RequireAPIPermission(store, models.PermScan)(handler.RequireAuth(handler.RulesValidate())))
	mux.HandleFunc("/api/rules/augment", handler.RequireAPIPermission(store, models.PermScan)(handler.RequireAuth(handler.RulesAugment(store))))
	...
	return handler.WithRequestID(handler.WithSecurityHeaders(handler.WithTrustedOrigin(mux)))
}
