...
func TestDeleteReportRejectsCrossSitePost(t *testing.T) {
	...
	h := New(store)
	cookie := loginAs(t, h, "admin", "admin-test-password")
...
func TestSettingsRejectsExternalRedirectTo(t *testing.T) {
	store := newServerTestStore(t)
	h := New(store)
	cookie := loginAs(t, h, "admin", "admin-test-password")
	body := url.Values{}
	body.Set("action", "save_blacklist")
	body.Set("redirect_to", "https://evil.example.com/phish")
	req := httptest.NewRequest(http.MethodPost, "/settings?tab=blacklist", strings.NewReader(body.Encode()))
	req.Host = "scanner.example.com"
	req.Header.Set("Origin", "https://scanner.example.com")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	req.AddCookie(cookie)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("expected settings page render, got %d", rec.Code)
	}
	if got := rec.Header().Get("Location"); got != "" {
		t.Fatalf("expected no external redirect, got %q", got)
	}
}

func TestSettingsBlacklistAddRequiresSaveToApply(t *testing.T) {
	store := newServerTestStore(t)
	h := New(store)
	cookie := loginAs(t, h, "admin", "admin-test-password")
	body := url.Values{}
	body.Set("action", "add_blacklist")
	body.Set("item_type", "domain")
	body.Set("item_value", "blocked.example.com")
	req := httptest.NewRequest(http.MethodPost, "/settings?tab=blacklist", strings.NewReader(body.Encode()))
	req.Host = "scanner.example.com"
	req.Header.Set("Origin", "https://scanner.example.com")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	...
	if items := store.ListPolicyBlacklist(); len(items) != 0 {
		t.Fatalf("expected active blacklist unchanged before save, got %+v", items)
	}
	if !store.IsPolicyBlacklistDraftDirty() {
		t.Fatalf("expected pending blacklist draft before save")
	}
	if drafts := store.ListPolicyBlacklistEntries(); len(drafts) != 1 || drafts[0].Value != "blocked.example.com" {
		t.Fatalf("expected blacklist item staged in draft, got %+v", drafts)
	}
}

func TestSettingsBlacklistRemoveRequiresSaveToApply(t *testing.T) {
	store := newServerTestStore(t)
	if err := store.AddPolicyBlacklistWithType("domain", "blocked.example.com"); err != nil {
		t.Fatalf("seed blacklist draft: %v", err)
	}
	if err := store.SavePolicyBlacklist(); err != nil {
		t.Fatalf("seed blacklist active: %v", err)
	}
	h := New(store)
	cookie := loginAs(t, h, "admin", "admin-test-password")
	body := url.Values{}
	body.Set("action", "remove_blacklist")
	body.Set("item_type", "domain")
	body.Set("item", "blocked.example.com")
	req := httptest.NewRequest(http.MethodPost, "/settings?tab=blacklist", strings.NewReader(body.Encode()))
	req.Host = "scanner.example.com"
	req.Header.Set("Origin", "https://scanner.example.com")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	...
	if items := store.ListPolicyBlacklist(); len(items) != 1 || items[0] != "blocked.example.com" {
		t.Fatalf("expected active blacklist unchanged before save, got %+v", items)
	}
	if !store.IsPolicyBlacklistDraftDirty() {
		t.Fatalf("expected pending blacklist draft removal before save")
	}
	if drafts := store.ListPolicyBlacklistEntries(); len(drafts) != 0 {
		t.Fatalf("expected blacklist item removed only in draft, got %+v", drafts)
	}
}

func TestSettingsRejectsInvalidUsername(t *testing.T) {
	store := newServerTestStore(t)
	h := New(store)
	cookie := loginAs(t, h, "admin", "admin-test-password")
	body := url.Values{}
	body.Set("action", "add_user")
	body.Set("username", "<script>")
	body.Set("password", "pass123")
	body.Set("team", "team-a")
	req := httptest.NewRequest(http.MethodPost, "/settings?tab=users", strings.NewReader(body.Encode()))
	req.Host = "scanner.example.com"
	req.Header.Set("Origin", "https://scanner.example.com")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	req.AddCookie(cookie)
	...
	if store.GetUser("<script>") != nil {
		t.Fatalf("expected invalid username not to be created")
	}
}

func TestDeletedUserSessionCannotAccessProtectedPage(t *testing.T) {
	store := newServerTestStore(t)
	if err := store.CreateUserWithTeam("deleted-user", "pass123", "team-a"); err != nil {
		t.Fatalf("create user: %v", err)
	}
	h := New(store)
	cookie := loginAs(t, h, "deleted-user", "pass123")
	if err := store.DeleteUser("deleted-user"); err != nil {
	...
	if rec.Code != http.StatusFound {
		t.Fatalf("expected redirect for deleted user session, got %d", rec.Code)
	}
