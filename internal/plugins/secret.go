...
func (p *SecretDetector) Metadata() Metadata {
	return Metadata{
		ID:           "secret",
		Name:         p.Name(),
		Description:  "硬编码凭据检测",
		Type:         PluginTypeDetector,
		Capabilities: []string{"secret-detection", "regex-scan"},
		HotSwappable: true,
	}
}

// Execute implements Plugin.
func (p *SecretDetector) Execute(ctx context.Context, req ExecuteRequest) (ExecuteResponse, error) {
