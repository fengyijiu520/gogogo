...
func (g *Generator) buildDocument(findings []plugins.Finding, score float64, modelInfo string, llmEnabled bool, result *review.Result, intent IntentSummary, profile AnalysisProfile) string {
	...
	if result != nil {
		...
		b.WriteString(para(fmt.Sprintf("沙箱执行摘要: source=%s；verdict=%s；score=%d；duration=%dms；fallback=%t", defaultText(result.Behavior.SandboxSource, "unknown"), defaultText(result.Behavior.SandboxVerdict, "unknown"), result.Behavior.SandboxScore, result.Behavior.SandboxDurationMs, result.Behavior.SandboxFallback), "22", "left", false))
		...
		b.WriteString(blankPara())
		b.WriteString(heading("外联目标与威胁情报", "28"))
		b.WriteString(para("本节按目标、信誉、来源和威胁类型展示情报结论，用于支撑外联风险复核。", "22", "left", false))
		if len(result.TIReputations) == 0 {
			b.WriteString(para("未检出可用的情报结果。", "22", "left", false))
		} else {
			for _, item := range result.TIReputations {
				line := fmt.Sprintf("- 目标=%s；信誉=%s；置信度=%.2f；来源=%s；威胁类型=%s；说明=%s", defaultText(item.Target, "unknown"), defaultText(item.Reputation, "unknown"), item.Confidence, defaultText(item.Source, "unknown"), defaultText(item.ThreatType, "n/a"), defaultText(item.Reason, "无"))
