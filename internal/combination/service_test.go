...
func TestBuildOverviewAggregatesSelectedSkills(t *testing.T) {
	...
	if !strings.Contains(overview.Conclusion.ClosureNarrative, "source") || !strings.Contains(overview.Conclusion.ClosureNarrative, "sink") {
		t.Fatalf("expected closure narrative mention closed path, got %+v", overview.Conclusion)
	}
...
func TestBuildOverviewFromRealisticFixtures(t *testing.T) {
	...
	} else if strings.TrimSpace(stored.Overview.ClosureNarrative) == "" {
		t.Fatalf("expected stored overview closure narrative, got %+v", stored.Overview)
	} else if len(stored.Overview.InferredChains) > 0 && len(stored.Overview.InferredChains[0].ClosureRequirements) == 0 {
		t.Fatalf("expected stored inferred chain closure requirements, got %+v", stored.Overview.InferredChains[0])
	}
...
func TestBuildSelectionKeyIsOrderInsensitive(t *testing.T) {
	left := buildSelectionKey([]string{"skill-c", "skill-a", "skill-b", "skill-a"})
	right := buildSelectionKey([]string{"skill-b", "skill-c", "skill-a"})
	if left == "" || right == "" {
		t.Fatalf("expected non-empty selection keys, left=%q right=%q", left, right)
	}
	if left != right {
		t.Fatalf("expected same key for same set regardless order/duplicates, left=%q right=%q", left, right)
	}
}
...
func TestBuildConclusionEmptySelection(t *testing.T) {
	conclusion := buildConclusion(nil, &admissionmodel.CapabilityProfile{}, nil, nil, tiRiskSummary{})
	...
	if !strings.Contains(conclusion.ClosureNarrative, "未选择足够技能") {
		t.Fatalf("expected pending closure narrative, got %+v", conclusion)
	}
}
...
func TestBuildConclusionKeepsPureCapabilityOverlapAtMedium(t *testing.T) {
	...
	conclusion := buildConclusion([]SkillOption{{SkillID: "a"}, {SkillID: "b"}}, profile, nil, nil, tiRiskSummary{})
	...
	if !strings.Contains(conclusion.ClosureNarrative, "runtime") {
		t.Fatalf("expected medium closure gaps narrative, got %+v", conclusion)
	}
}
...
func TestBuildConclusionUsesHighConfidenceChainsForHighRisk(t *testing.T) {
	...
	conclusion := buildConclusion([]SkillOption{{SkillID: "a"}, {SkillID: "b"}}, profile, nil, chains, tiRiskSummary{})
	...
	if !strings.Contains(conclusion.ClosureNarrative, "source-sink") {
		t.Fatalf("expected closed-loop narrative, got %+v", conclusion)
	}
}

func TestBuildConclusionUsesChainClosureRequirements(t *testing.T) {
	profile := &admissionmodel.CapabilityProfile{NetworkAccess: true, CommandExec: true}
	chains := []InferredChain{{
		ID:                  "remote-command-chain",
		Level:               "high",
	...
	conclusion := buildConclusion([]SkillOption{{SkillID: "a"}, {SkillID: "b"}}, profile, nil, chains, tiRiskSummary{})
	if strings.Contains(conclusion.ClosureNarrative, "transform") {
		t.Fatalf("expected closure narrative respects rule requirements and skips transform gap, got %+v", conclusion)
	}
}
...
func TestInferChainsRegressionSamples(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "采集与外联可触发采集后外发链",
			selected: []selectedSignal{
				{Option: SkillOption{SkillID: "A", DisplayName: "Skill A"}, Profile: &admissionmodel.CapabilityProfile{DataCollection: true, Evidence: []string{"collect user profile", "harvest items"}}},
				{Option: SkillOption{SkillID: "B", DisplayName: "Skill B"}, Profile: &admissionmodel.CapabilityProfile{NetworkAccess: true, Evidence: []string{"https://example.com/upload"}}},
			},
			profile:   &admissionmodel.CapabilityProfile{DataCollection: true, NetworkAccess: true},
			wantChain: []string{"collection-exfiltration-chain"},
		},
		{
			name: "外部拉取与执行可触发下载后执行链",
			selected: []selectedSignal{
				{Option: SkillOption{SkillID: "A", DisplayName: "Skill A"}, Profile: &admissionmodel.CapabilityProfile{ExternalFetch: true, Evidence: []string{"download payload", "https://example.com/payload.sh"}}},
				{Option: SkillOption{SkillID: "B", DisplayName: "Skill B"}, Profile: &admissionmodel.CapabilityProfile{CommandExec: true, Evidence: []string{"exec.Command", "run payload"}}},
			},
			profile:   &admissionmodel.CapabilityProfile{ExternalFetch: true, CommandExec: true},
			wantChain: []string{"fetch-exec-chain"},
		},
	}
...
func TestChainRuleCoverageCount(t *testing.T) {
	rules := getChainRules()
	if len(rules) < 20 {
		t.Fatalf("expected at least 20 chain rules, got %d", len(rules))
	}
}
