...
func (o *Orchestrator) Run(in Input) (review.Result, error) {
	...
	result.Pipeline = append(result.Pipeline, review.PipelineStage{
		...
		Purpose:    "执行 zeroclaw 沙箱分析（strace、网络监控、行为监控、SKILL.md 分析）",
		Status:     "running",
		Input:      in.ScanPath,
		Benefit:    "把静态规则看不到的真实运行行为纳入用户决策证据链",
		MethodNote: "使用 zeroclaw 沙箱进行完整行为分析，包括 strace 系统调用追踪、网络抓包、进程监控",
	})

	// 使用完整的 zeroclaw 沙箱扫描
	scanResult, scanErr := sandbox.RunZeroclawScan(in.Context, sandbox.ZeroclawScanRequest{
		SkillPath:   in.ScanPath,
		SkillName:   in.Description,
		RequestID:   in.RequestID,
		Timeout:     0,
		LLMProvider: in.LLMProvider,
		LLMProtocol: in.LLMProtocol,
	...
	if scanErr != nil {
		...
		result.Pipeline[len(result.Pipeline)-1].Status = "failed"
		result.Pipeline[len(result.Pipeline)-1].Output = scanErr.Error()
		return result, fmt.Errorf("沙箱行为分析执行失败，已阻断扫描，请检查沙箱镜像、运行时和待扫描文件后重试: %w", scanErr)
	}
	behavior := scanResult.ConvertToBehaviorProfile()
	behavior.SandboxDurationMs = scanResult.TotalDurationMs
	result.Behavior = behavior

	// 收集 IoC 用于威胁情报查询
	var iocs []string
	iocs = append(iocs, behavior.OutboundIOCs...)
	iocs = append(iocs, behavior.ExecuteIOCs...)
	iocs = append(iocs, behavior.CredentialIOCs...)
	iocs = uniqueStrings(iocs)

	applyStaticBehaviorCrossChecks(&result.Behavior, in.BaseFindings)
	result.Evasion = assessEvasion(result.Behavior)
	result.Pipeline[len(result.Pipeline)-1].Status = "completed"
	result.Pipeline[len(result.Pipeline)-1].Output = fmt.Sprintf("zeroclaw 分析完成: 行为证据类别 %d 类，IoC %d 个；sandbox=%s/%s score=%d duration=%dms", evidence.CountBehaviorCategories(result.Behavior), len(iocs), result.Behavior.SandboxSource, result.Behavior.SandboxVerdict, result.Behavior.SandboxScore, result.Behavior.SandboxDurationMs)
	result.Pipeline = append(result.Pipeline, review.PipelineStage{
		Name:       "intent_compare",
	...
	_, diffs := o.intent.Evaluate(in.Description, in.Permissions, result.Behavior)
	...
	behaviorDeduction, behaviorVeto, behaviorReason := assessBehaviorRisk(result.Behavior)
	...
	high, medium, low := deriveCalibratedRiskCounts(result.Findings, diffs, reputations, result.Behavior, result.Evasion, malicious, behaviorVeto)
	...
	result.EvidenceInventory = evidence.BuildInventory(result.Behavior, diffs, reputations, result.Evasion)
...
func appendUniqueWarningText(items []string, warning string) []string {
...
func assessBehaviorRisk(behavior review.BehaviorProfile) (float64, bool, string) {
	...
	veto := false
	hasBehaviorChains := len(behavior.BehaviorChains) > 0
	...
	criticalAlerts, contextualAlerts := classifySequenceAlerts(behavior.SequenceAlerts)
	if len(criticalAlerts) > 0 {
		deduction += minFloat(12, float64(len(criticalAlerts))*3)
		reasons = append(reasons, "命中关键恶意行为时序")
	}
	if len(contextualAlerts) > 0 && !hasBehaviorChains {
		deduction += minFloat(6, float64(len(contextualAlerts))*2)
		reasons = append(reasons, "命中上下文行为时序")
	}

	if len(behavior.ProbeWarnings) > 0 {
		deduction += minFloat(6, float64(len(behavior.ProbeWarnings)))
		reasons = append(reasons, "动态执行覆盖存在缺口")
	}
...
func classifySequenceAlerts(alerts []string) ([]string, []string) {
	critical := make([]string, 0, len(alerts))
	contextual := make([]string, 0, len(alerts))
	for _, alert := range alerts {
		...
		switch alert {
		case "命中凭据访问后外联时序", "命中防御规避后执行时序", "命中横向移动联动控制时序":
			critical = append(critical, alert)
		default:
			contextual = append(contextual, alert)
		}
	}
