...
func TestClassifySequenceAlerts(t *testing.T) {
	critical, contextual := classifySequenceAlerts([]string{"命中下载后执行时序", "命中凭据访问后外联时序", "命中横向移动联动控制时序"})
	if len(critical) != 2 {
		t.Fatalf("expected 2 critical alerts, got %+v", critical)
	}
	if len(contextual) != 1 || contextual[0] != "命中下载后执行时序" {
		t.Fatalf("expected download-execute to stay contextual, got %+v", contextual)
	}
}
