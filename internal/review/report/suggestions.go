...
func BuildDynamicSuggestions(findings []plugins.Finding, refined review.Result) []string {
	...
	for _, f := range orderedFindings {
		if shouldSkipRemediationSuggestionFinding(f) {
			continue
		}
		severity := localizeSeverity(f.Severity)
		location := remediationLocation(f.Location)
		switch {
		case strings.HasPrefix(strings.ToUpper(strings.TrimSpace(f.RuleID)), "V7-00") || f.RuleID == "V7-010" || f.RuleID == "V7-011" || f.RuleID == "V7-012" || f.RuleID == "V7-013" || f.RuleID == "V7-014":
			add(fmt.Sprintf("针对规则 %s（%s）在 %s 执行强制修复：移除高危调用或增加显式权限校验，并补充单元测试覆盖该路径。", f.RuleID, f.Title, defaultIfEmpty(location, "代码实现")))
		case f.RuleID == "V7-003":
			add(fmt.Sprintf("将外联目标 %s 收敛到受控白名单域名，禁止明文 HTTP，必要时增加签名校验与下载完整性校验。", defaultIfEmpty(location, "未知目标")))
		...
		case f.RuleID == "RULE-AUTO-COVERAGE":
			add("优先补齐当前规则集中可自动评估但尚未覆盖的规则映射，确保系统评估覆盖全部自动项后再进行完整整改判断。")
		case f.RuleID == "V7-006":
			add("根据声明与行为偏差，更新技能设计说明与权限声明，确保行为路径与声明能力一一对应，并删除未声明能力代码。")
		default:
			switch severity {
			case "高风险":
				add(fmt.Sprintf("修复高风险项 %s：在 %s 增加输入校验、最小权限控制与异常处理，避免高危能力在无防护条件下执行。", defaultIfEmpty(f.Title, f.RuleID), defaultIfEmpty(location, "相关模块")))
			case "中风险":
				add(fmt.Sprintf("修复中风险项 %s：在 %s 补充边界校验、权限收敛和日志审计，防止风险在特定条件下放大。", defaultIfEmpty(f.Title, f.RuleID), defaultIfEmpty(location, "相关模块")))
			case "低风险":
				add(fmt.Sprintf("处理低风险项 %s：在 %s 完善配置与防护细节，并在下个迭代完成复测闭环。", defaultIfEmpty(f.Title, f.RuleID), defaultIfEmpty(location, "相关模块")))
			}
		}
	}
...
func shouldSkipRemediationSuggestionFinding(f plugins.Finding) bool {
	joined := strings.ToLower(strings.Join([]string{f.Location, f.CodeSnippet, f.Description, f.Title}, " "))
	if strings.Contains(joined, ".scan-cache.json") || strings.Contains(joined, "行为证据摘要") {
		return true
	}
	if isReportSkippableDocumentationText(joined) && !strings.EqualFold(strings.TrimSpace(f.RuleID), "V7-006") {
		return true
	}
	return false
}

func remediationLocation(location string) string {
	trimmed := strings.TrimSpace(location)
	lower := strings.ToLower(trimmed)
	if trimmed == "" || strings.Contains(lower, ".scan-cache.json") || isReportSkippableDocumentationText(lower) {
		return "相关模块"
	}
	return trimmed
}

func isReportDocumentationExampleText(text string) bool {
	lower := strings.ToLower(strings.TrimSpace(text))
	if lower == "" {
		return false
	}
	return strings.Contains(lower, "skill.md") || strings.Contains(lower, "readme") || strings.Contains(lower, "docs/") || strings.Contains(lower, "examples/") || strings.Contains(lower, "example/") || strings.Contains(lower, "示例") || strings.Contains(lower, "文档")
}

func isReportSkippableDocumentationText(text string) bool {
	lower := strings.ToLower(strings.TrimSpace(text))
	if lower == "" {
		return false
	}
	if strings.Contains(lower, "readme") || strings.Contains(lower, "docs/") || strings.Contains(lower, "examples/") || strings.Contains(lower, "example/") {
		return true
	}
	if strings.Contains(lower, "skill.md") {
		return isReportSkillExampleOrPlaceholderText(lower)
	}
	return strings.Contains(lower, "示例") || strings.Contains(lower, "文档")
}

func isReportSkillExampleOrPlaceholderText(text string) bool {
	lower := strings.ToLower(strings.TrimSpace(text))
	if lower == "" || !strings.Contains(lower, "skill.md") {
		return false
	}
	if strings.Contains(lower, "<url>") || strings.Contains(lower, "[slug]") || strings.Contains(lower, "[path]") || strings.Contains(lower, "example.com") || strings.Contains(lower, "example.org") || strings.Contains(lower, "example.net") {
		return true
	}
	return (strings.Contains(lower, "示例") || strings.Contains(lower, "example") || strings.Contains(lower, "sample") || strings.Contains(lower, "demo") || strings.Contains(lower, "安装教程")) && (strings.Contains(lower, "curl ") || strings.Contains(lower, "wget ") || strings.Contains(lower, "git clone") || strings.Contains(lower, "pip install") || strings.Contains(lower, "npm install"))
}
